...
private:
    ...
    Eigen::Affine3d des_gripper1_affine_wrt_twopsm_,des_gripper2_affine_wrt_twopsm_;
...
js_action_client_("trajActionServer", true)
{
   ...
   // attempt to connect to the server:
    ...
    tf::StampedTransform tfResult_two;    
    tf::StampedTransform tf_init_gripper1,tf_init_gripper2; 
    Eigen::Affine3d affine_base_to_camera_two,affine_gripper_wrt_base;
   bool tferr=true;
        ...
        try {
            //The direction of the transform returned will be from the target_frame to the source_frame. 
             //Which if applied to data, will transform data in the source_frame into the target_frame. See tf/CoordinateFrameConventions#Transform_Direction
                tfListener.lookupTransform("two_psm_base_link","camera", ros::Time(0), tfResult_two);
                tfListener.lookupTransform("two_psm_base_link","one_tool_tip_link",  ros::Time(0), tf_init_gripper1);                
                tfListener.lookupTransform("two_psm_base_link","two_tool_tip_link",  ros::Time(0), tf_init_gripper2); 
...
void CartMoveActionServer::executeCB(const actionlib::SimpleActionServer<cwru_action::cart_moveAction>::GoalConstPtr& goal) {
   ...
   des_gripper1_affine_wrt_twopsm_ = transformPoseToEigenAffine3d(des_pose_gripper1.pose);
   cout<<"gripper1 desired pose;  "<<endl;
   cout<<des_gripper1_affine_wrt_twopsm_.linear()<<endl;
   cout<<"origin: "<<des_gripper1_affine_wrt_twopsm_.translation().transpose()<<endl;

   des_gripper2_affine_wrt_twopsm_ = transformPoseToEigenAffine3d(des_pose_gripper2.pose);
   cout<<"gripper2 desired pose;  "<<endl;
   cout<<des_gripper2_affine_wrt_twopsm_.linear()<<endl;
   cout<<"origin: "<<des_gripper2_affine_wrt_twopsm_.translation().transpose()<<endl;
   ...
   //do IK to convert these to joint angles:
    ...
    des_gripper_affine1_ = gripper1_affine_last_commanded_pose_; //previous pose
    ...
    des_gripper_affine2_ = gripper2_affine_last_commanded_pose_; //previous pose
    ...
    des_gripper_affine1_ = des_gripper1_affine_wrt_twopsm_;
    ...
    des_gripper_affine2_ = des_gripper2_affine_wrt_twopsm_;
    ik_solver_.ik_solve(des_gripper_affine2_); //convert desired pose into equiv joint displacements
  ...
  //              Client::SimpleFeedbackCallback());
    ...
    double t_timeout=arrival_time_+15.0; //wait 2 sec longer than expected duration of move
    ...
    gripper1_affine_last_commanded_pose_ = des_gripper1_affine_wrt_twopsm_;
    gripper2_affine_last_commanded_pose_ = des_gripper2_affine_wrt_twopsm_;    
...
int main(int argc, char** argv) {
   ...
   ROS_INFO("getting transforms from camera to PSMs");
    ...
    Eigen::Affine3d des_gripper1_affine_wrt_twopsm,des_gripper2_affine_wrt_twopsm;
   bool tferr=true;
    ROS_INFO("waiting for tf between base and right_hand...");
    while (tferr) {
        tferr=false;
