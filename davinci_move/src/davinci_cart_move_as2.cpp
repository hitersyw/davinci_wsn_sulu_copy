...
{ 
        g_q_vec(0) = jointState.position[0+13];
        g_q_vec(1) = jointState.position[1+13];
        g_q_vec(2) = jointState.position[7+13];
        g_q_vec(3) = jointState.position[8+13];          //rotation about tool shaft:     
        g_q_vec(4) = jointState.position[9+13];        // wrist bend:
        g_q_vec(5) = jointState.position[10+13];       // q_vec[5] rotates both jaws together
...
void CartMoveActionServer::executeCB(const actionlib::SimpleActionServer<cwru_action::cart_moveAction>::GoalConstPtr& goal) {
   ...
   //do IK to convert these to joint angles:
    ...
    Vectorq7x1 q_vec1,q_vec2, q_vec2_jstate;
    q_vec1.resize(7);
    q_vec2.resize(7);
   ...
   Eigen::Affine3d affine_gripper_wrt_psm2, affine_gripper_wrt_psm2_FK;
   Eigen::Vector3d tip_from_FK_of_IK;
   ROS_INFO("FK of IK soln: ");
   affine_gripper_wrt_psm2 = davinci_fwd_solver.fwd_kin_solve(q_vec2);
   cout<<"affine linear (R): "<<endl;
   cout<<affine_gripper_wrt_psm2.linear()<<endl;
   tip_from_FK_of_IK = affine_gripper_wrt_psm2.translation();
   cout<<"origin: ";
   cout<<tip_from_FK_of_IK.transpose()<<endl;   
   cout<<"origin in left camera frame: ";
   cout<<affine_lcamera_to_psm_two_*tip_from_FK_of_IK<<endl;   
  ...
  //              Client::SimpleFeedbackCallback());
   ...
   affine_gripper_wrt_psm2_FK = davinci_fwd_solver.fwd_kin_solve(q_vec2_jstate);
   cout<<"affine linear (R): "<<endl;
   cout<<affine_gripper_wrt_psm2_FK.linear()<<endl;
   tip_from_FK = affine_gripper_wrt_psm2_FK.translation();
